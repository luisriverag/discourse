...
after_initialize do
  ...
  add_to_class(Middleware::AnonymousCache, :is_ie) do
    unless defined?(@is_ie)
      ...
      @is_ie = BrowserDetection.browser(@env[USER_AGENT]) == :ie
    end

    @is_ie
  end

  Middleware::AnonymousCache.cache_key_segments[:ie] = ->(h) { h.is_ie? }
